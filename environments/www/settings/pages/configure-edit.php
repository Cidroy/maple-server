<?php
$config = \maple\www\WWW::configuration($_REQUEST["edit"]);
$actives = \maple\www\WWW::sites("active");
if(!in_array($_REQUEST["edit"],array_keys($actives))) { header("Location: ".ENVIRONMENT::url()->current()."?".http_build_query([ "page" =>	"configure"])); die(); }
if(!$config){
...
<h2 class="text-primary">
	<form action="<?php echo \maple\environments\eWWW::url("settings")?>" method="post">
		<input type="text" name="location" value="<?php echo $_REQUEST["edit"]?>" hidden="true">
		<button type="submit" class="btn btn-danger pull-right"><span class="glyphicon glyphicon-remove"></span>Disable</button>
		<input type="text" name="www-ajax-action" value="disable-site" hidden="true">
	</form>
	Activate Website - <?php echo $_REQUEST["edit"]?>
</h2>
<form action="<?php echo \maple\environments\eWWW::url("settings")?>" method="post">
	<div class="panel">
		<div class="panel-body">
			<table class="table">
				<tbody>
					...
					<tr> <td><strong>Folder : </strong></td><td><?php echo $_REQUEST["edit"]?></td> </tr>
		...
		<div class="panel-footer">
			<input type="text" name="location" value="<?php echo $_REQUEST["edit"]?>" hidden="true">
			<input type="text" name="www-ajax-action" value="edit-site" hidden="true">
			<button type="submit" class="btn btn-success"><span class="glyphicon glyphicon-ok"></span>Edit</button>
		</div>
	</div>
</form>
